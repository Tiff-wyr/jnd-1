<template>
  <div class="secure-center">
    ...
    <toggleItem :title="'账户密码'" :is-show="updatePasswordShow" @click="handlePassword">
      当前密码强度：<span class="password-level">{{ nowLevel === 1 ? '弱' : nowLevel === 2 ? '中' : nowLevel === 3 ? '强' : '弱' }}</span>
      <div slot="hidden">
      ...
      <div v-if="isNext" slot="hidden">
        <el-form ref="phoneForm" :model="phoneForm" :rules="phoneFormRules" label-position="right" label-width="120px" style="margin-left: 100px;">
          ...
          <el-form-item label="密码" prop="password">
            <el-input v-model="phoneForm.password" class="w180" type="password"/>
          </el-form-item>
          <el-form-item>
...
export default {
  ...
  methods: {
    ...
    getPersonal() {
      this.$axios.get(`/userBorrower/updateBorrowerById/${this.id}`).then(res => {
