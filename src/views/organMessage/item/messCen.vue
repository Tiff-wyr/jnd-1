      ...
      <el-tabs>
            ...
            <div v-if="isInbox" class="inbox">
              ...
              >
                ...
                <el-table-column label="内容" align="center">
                  <template slot-scope="scope">
                    <div
                      ...
                      @click="lookShouRow(scope.row.id)"
                ...
                <!-- <el-table-column label="操作" width="150">
                  <template slot-scope="scope">
                    <el-button size="mini" type="danger" @click="deleteShouRow(scope.row.id)">删除</el-button>
                    <el-button size="mini" type="success" @click="lookShouRow(scope.row.id)">查看</el-button>
                  </template>
                </el-table-column>-->
              </el-table>
            </div>
        ...
        <el-tab-pane>
          ...
          <div class="outbox">
            ...
            >
              <el-table-column type="selection" align="center" width="55"></el-table-column>
              <el-table-column label="收件人" align="center" width="200">
                <template slot-scope="scope">
                  <span
                    style="overflow: hidden; white-space: nowrap;text-overflow: ellipsis"
                  >{{scope.row.toName}}</span>
                </template>
              </el-table-column>
              <el-table-column label="内容" align="center">
                <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    getSMessage() {
      ...
      this.$axios
        .get(
          `/message/getPageReceiveMessage/${this.$store.state.userInfo.phone}`
        )
        .then(res => {
    ...
    getFMessage() {
      this.listLoading = true;
      this.$axios
        .get(`/message/getPageSendMessage/${this.$store.state.userInfo.phone}`)
        .then(res => {
    ...
    deleteShouRow(id) {
      this.$axios
        .get(
          `message/removeReceiveMessage/${id}/${
    ...
    subInbox() {
      if (this.selectMultipleInbox.length > 0) {
        ...
        this.$axios
          .get(
            `/message/removeReceiveMessageBatch/${this.inbox}/${
              this.$store.state.userInfo.phone
            }`
          )
          .then(res => {
    ...
    subOutbox() {
      if (this.selectMultipleOutbox.length > 0) {
        ...
        this.$axios
          .get(
            `message/removeSendMessageBatch/${this.outbox}/${
              this.$store.state.userInfo.phone
            }`
          )
          .then(res => {
